...
private:
    ...
    //Compression_D_Box C_D_boxes[16];
    int IP[INPUT_SIZE] =
        {
...
{
    //cout << "apply expansion" << endl;
    bitset<ROUND_KEY_SIZE> output;
    for (int i = 0; i < ROUND_KEY_SIZE; i++)
    {
        output[i] = input[expansion_permutation[i]-1];
    }
    return output;
}
...
void DES::DES_Encryption()
{
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        // Input to next round is output of current round
        // cout << "\t\tOutput of Round " << i+1 << " is " << out << endl;
        in = out;
    }
    // Final Permutation
    //cout << "OUT = " << out << endl;
    apply_permutation(out, FP);
    cout << "Encryption is: " << out.to_string() << endl;
    _output = out;
}
...
void DES::DES_Decryption()
{
    ...
    apply_permutation(out, FP);
    cout << "Decryption is: " << out.to_string() << endl;
    _input = out; 
}
...
{
    //cout << "generate Round Keys" << endl;
    bitset<56> updated_key;
    // Parity Drop
    int j = 0;
    for (int i = 0; i < KEY_SIZE; i++)
    {
        if ((i + 1) % 8 != 0)
        {
            updated_key.set(j, DES_key[i]);
            j++;
        }
    }
    //cout << "Key updated: " << updated_key << endl;
    // Feeding the updated key now.
    // For 16 iterations create Round key and store it in Round_key array. Assuming shift-left by 1 on 1,2,9,16.
    
    bitset<56> *temp;
    bitset<56> divider(0b00000000000000000000000000001111111111111111111111111111);
    // Split the input in two halves
    bitset<28> left(((updated_key >> 28) & divider).to_string().substr(28,28));
    bitset<28> right((updated_key & divider).to_string().substr(28, 28));
    //cout << "Left: " << left << "\nRight: " << right << endl;
    for (int i = 0; i < 16; i++)
    {
...
{
    //cout << "Swapper\n";
    // Swapping is nothing but rotating by half the length of input stream
    int len = INPUT_SIZE;
...
bitset<INPUT_SIZE> DES::mixer(bitset<INPUT_SIZE> &input, bitset<ROUND_KEY_SIZE> &round_key)
{
    ...
    //cout << "\t\tMixer output = " << mixer_output << endl;
    return mixer_output;
}
...
bitset<INPUT_SIZE / 2> DES::DES_function(bitset<32> &R, bitset<ROUND_KEY_SIZE> &round_key)
{
    ...
    // Expnad the 'R' input by passing it through Expansion D box
    ...
    for(int i = 0; i < 8; i++){
        substitutions[i] = get_substitution(splits[i], S_boxes[i]);
    }
    //cout << "All substitutions obtained!" << endl;
    /*for(int i = 0; i < 8; i++){
        cout << "#" << i+1 << " " << substitutions[i].to_string() << endl;
    }*/
    string sub = "";
    for(int i = 0; i < 8; i++)
    {
    ...
    //cout << "sub = " << sub << endl;
    bitset<32>* temp = new bitset<32>(sub);
    intermediate2 = *temp;
    delete temp;
    // Apply P-box
...
int main()
{
    cout << "Hello World of Cryptography!" << endl;
    bitset<INPUT_SIZE> input(0x0494720FC08E053D);
    bitset<KEY_SIZE> DESKey(0x0000000000000000);
    cout << "Input: " << input << endl;
    cout << "DES Key: " << DESKey << endl;
    DES obj(input, DESKey, true);
